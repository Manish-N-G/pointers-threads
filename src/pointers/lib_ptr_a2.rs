...
//!
//! Make sure you have the change to brush up on how the following work 
...
//! struct a number type, so this way, it becomes a generic over
...
impl<T: MyNums> MySelfRefState<T> {
    ...
    pub fn put_ptr(self) -> std::pin::Pin<Box<MySelfRefStatePin<T>>> {
        let _my_ref = MySelfRefStatePin { 
            val: self.val,
            ptr: std::ptr::null(),
            // cause of non-null, I would have to look for something
...
/// a different memory address, via the help of Pinning
///
...
impl<T: MyNums> MySelfRefStatePin<T> {
    ...
    // to fine the get_addresses function. We don't use compile_fail, but
    ...
    /// ```
    /// # Safety
    /// Here the Safety argument is that, we will be able to 
    ...
    /// we dereference raw pointers. Otherwise, we are able to 
