...
// this means that files that point to main.rs don't expose the 
...
// I block on `mod3`_a directly to avoid changing main via `tokio`::main
// #[`tokio`::main]
fn main() {
    // my_`func`!('`m',` 1, c);
    ...
    //
    // my_`func`!('`m',` 2, a);
    // my_`func`!('`m',` 1, g).await;
    //
    // `mod1`_b();
    // main_test_SelfRef();

    // `thread1c`_mutex_lock_attempt_drop(std::sync::`Mutex`::new(`100u8`), true, true, true);
    // `thread1c`_mutex_lock_attempt_drop(std::sync::`Mutex`::new(`100u8`), true, false, true);
    //
    // `thread1c`_mutex_lock_attempt(true, 33); 
    // `thread1c`_mutex_hangs_forever( 5, true );
    ...
    // `println`!( "vec is :");
    // for vec in `thread1c`_arc_mutex_display( &["display", "move", "loop -1"], 100_000, false) {
    //     `println`!( "{}", vec );
    // }
    //
...
fn mod1_a() {
    // I don't need to do any more: use pointers_threads::lib_`th1a`::*;
    // Cause I have `threads1` mod exposing lib_`th1a` functions
    // But I still prefer to do it so that I can test based on the module.
    // use `threads1`::*;
    // use pointers_threads::lib_`th`_a::*;
    // use pointers_threads::`thread1st`_get_current;
    ...
    // This also works
    // `println`!("val for the scope vec is {:?}", 
    //     `thread1a`_scope_vec::<std::ops::Range<i32>, std::ops::Range<char>, `i32`, char>(
    //         (1..1000), ('`a'.`.'j'), '`j',` true
    //     )
    // );
...
fn mod1_b() {
    // use pointers_threads::lib_`th`_b::*;
...
fn mod1_g() {
    // let x = `th1`::`th1g`::`state1`_`oop`_non_`oop`::Post_`OOP`::new();
...
// to do it in the `mod3`_a function which prevent me from writing `tokio`::main
// async fn `mod3`_a() {
//     `th3`::`th3a`::`thread3a`_normal_async().await;
//     `th3`::`th3a`::`thread3a`_normal_async().await;
// }
fn mod3_a() {
...
// `eg`: #[cfg(test)] in lib.rs or lib module via pub(create) or within
...
// Can have #[cfg(test)] and #[test], but we don't need to really specify
...
//
// For Binary crate:
// In the case for Binary crate, rust creates a separate crate for each 
